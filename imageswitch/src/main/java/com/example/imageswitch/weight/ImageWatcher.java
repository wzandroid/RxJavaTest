...
import com.example.imageswitch.R;
...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    protected int mErrorImageRes = R.mipmap.ic_launcher; // 图片加载失败站位图
    ...
    protected List<String> initUrlList;
    ...
    protected List<String> mUrlList; // 图片地址列表
    ...
    public interface Loader {
        void load(Context context, String uri, LoadCallback lc);
    }
    ...
    public interface IndexProvider {
        ...
        void onPageChanged(ImageWatcher imageWatcher, int position, List<String> dataList);
    }
    ...
    public class DefaultIndexProvider implements IndexProvider {
        ...
        @Override
        public void onPageChanged(ImageWatcher imageWatcher, int position, List<String> dataList) {
            if (mUrlList.size() > 1) {
    ...
    public interface OnStateChangedListener {
        void onStateChangeUpdate(ImageWatcher imageWatcher, ImageView clicked, int position, String uri, float animatedValue, int actionTag);

        void onStateChanged(ImageWatcher imageWatcher, int position, String uri, int actionTag);
    }
    ...
    public interface OnPictureLongPressListener {
         ...
         */
        void onPictureLongPress(ImageView v, String uri, int pos); // 当前展示图片长按的回调
    }

    public void show(List<String> urlList, int initPos) {
        if (urlList == null) {
     ...
     */
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, final List<String> urlList) {
    ...
    private void showInternal(ImageView i, SparseArray<ImageView> imageGroupList, final List<String> urlList) {
        if (loader == null) {
